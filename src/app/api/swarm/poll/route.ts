...
export async function POST(request: NextRequest) {
  try {
    ...
    if (statsResult.ok && typeof statsResult.data === 'object' && statsResult.data !== null &&
      'success' in statsResult.data && (statsResult.data as { success: boolean }).success
    ) {
...
export async function GET(request: NextRequest) {
  ...
  if ('swarmId' in swarm && typeof swarm.swarmId === 'string' && swarm.swarmId) {
    ...
    if (detailsResult.ok && detailsResult.data !== null && 'success' in detailsResult.data) {
      const details = detailsResult.data as { data?: { x_api_key?: string } };
      const xApiKey = details.data?.x_api_key;
      const swarm_id = (swarm as { swarmId?: string; id: string }).swarmId || swarm.id;
      // Extract the numeric part from swarm_id using regex
      const match = typeof swarm_id === 'string' ? swarm_id.match(/(\d+)/) : null;
