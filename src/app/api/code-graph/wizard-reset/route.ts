...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session?.user) {
      ...
      } as WizardResetResponse, { status: 401 });
    }
    ...
    if (!workspaceSlug) {
      ...
      } as WizardResetResponse, { status: 400 });
    }
    ...
    if (!validatedSlug) {
      ...
      } as WizardResetResponse, { status: 403 });
    }
    ...
    if (!workspace) {
      ...
      } as WizardResetResponse, { status: 404 });
    }
    ...
    } else if (typeof resetSwarm.wizardData === 'object' && resetSwarm.wizardData !== null) {
    ...
    } as WizardResetResponse, { status: 500 });
  }
}