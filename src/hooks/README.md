...
This directory contains focused hooks.
...
function MyComponent() {
  const { workspace, slug, role, switchWorkspace, isOwner } = useWorkspace();
...
function MyComponent() {
  const { canWrite, permissions, checkPermission, getAccessLevel } =
    useWorkspaceAccess();
